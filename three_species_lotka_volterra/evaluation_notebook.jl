...
@bind problem_name Radio(["Boehm", "Ruoff", "three_species_lotka_volterra"];default="three_species_lotka_volterra")
...
begin 
	df.λ_reg = string.(df[:,"λ_reg"]);
	df.hidden_layers = string.(df[:,"hidden_layers"]);
	df.hidden_neurons = string.(df[:,"hidden_neurons"]);
	df.tolerance = string.(df[:,"tolerance"]);
	df.epochs_adam = string.(df[:,"epochs_adam"]);
	df.epochs_bfgs = string.(df[:,"epochs_bfgs"]);
end;
...
@bind y_axis Select(y_options; default="loss")
...
begin
	p = Plot(df, x=x, y=y, kind="box", Layout(width=700, height=300))
end
...
md"""First Parameter for Heatmap
$(@bind Parameter1 Select(x_options;default="λ_reg"))"""

# ╔═╡ e8644c68-edb4-4fce-aa1a-2c8a50380110
md"""Second Parameter for Heatmap
$(@bind Parameter2 Select(x_options;default="act_fct"))"""

# ╔═╡ db1accb9-e51a-483e-8d4a-4ea690614db9
md"""Parameter to compare for Heatmap
$(@bind Parameter3 Select(y_options;default="loss"))"""

# ╔═╡ ea1e046d-bf54-43b9-a658-eb4b404d0b02
begin
	Parameter_1 = df[!,[Parameter1]]
	Parameter_2 = df[!,[Parameter2]]
	Parameter_3 = unstack(df, Symbol(Parameter1), Symbol(Parameter2), Symbol(Parameter3), combine=mean)
end
