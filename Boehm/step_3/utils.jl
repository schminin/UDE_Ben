...
"""
function nll(θ, IC::Vector{Float64}=IC, solver=solver, t::Vector{Float64}=t, y_obs::Matrix{Float64}=y_obs, y_hidden::Matrix{Float64}=y_hidden; st_nn::NamedTuple = st, nn_model::Chain = nn_model)
    ...
    if size(tmp_sol) == size(y_hidden) # see https://docs.sciml.ai/SciMLSensitivity/stable/tutorials/training_tips/divergence/
        @inbounds X̂ = Array(tmp_sol)
        pred = observable_mapping(X̂, θ, st_nn; augmented_dynamics=nn_model)
        # loss = n_t * log(sigma) + 1/sigma² * sum_t (pred-true)² = lc1 + lc2
        log_sigma_sq = [θ.n_pSTAT5A_rel, θ.n_pSTAT5B_rel, θ.n_rSTAT5A_rel]
        lc_1 = size(y_obs)[2]/2 * log_sigma_sq
        lc_2 = sum(abs2, pred.-y_obs, dims=2) ./ exp.(log_sigma_sq)
...
function train_Boehm(p::ComponentVector, st::NamedTuple, lr_adam::Float64, λ_reg::Float64, stepnorm_bfgs::Float64, epochs::Tuple{Int, Int}, l_mech::Int)
    # Container to track the training
    losses = Float64[];
    losses_regularization = Float64[];
    Epo_degradation_BaF3 = Float64[];
    k_exp_hetero = Float64[];
    k_exp_homo = Float64[];
    k_imp_hetero = Float64[];
    k_imp_homo = Float64[];
    k_phos = Float64[];
    ...
    callback = function (p, l)
        ...
        @ignore_derivatives push!(Epo_degradation_BaF3, @inbounds p[1])
        @ignore_derivatives push!(k_exp_hetero, @inbounds p[2])
        @ignore_derivatives push!(k_exp_homo, @inbounds p[3])
        @ignore_derivatives push!(k_imp_hetero, @inbounds p[4])
        @ignore_derivatives push!(k_imp_homo, @inbounds p[5])
        @ignore_derivatives push!(k_phos, @inbounds p[6])
    ...
    return res2.u, st, losses, losses_regularization, Epo_degradation_BaF3, k_exp_hetero, k_exp_homo, k_imp_hetero, k_imp_homo, k_phos
end
